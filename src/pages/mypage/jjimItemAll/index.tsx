...
import JjimItemMore from '@/components/mypage/productList/JjimItemMore'
    ...
    <>
      <div className={styles.container}>
        ...
        <section className={styles.section}>
          <span className={styles.title}>찜한 상품</span>
          <hr className={styles.line} />
          <div>
            <ul className={styles.jjimItem}>
              {jjimItem.map((item: Product, index: number) => (
                <JjimItemMore key={index} item={item} onRemove={() => handleRemoveCartItem(index)} />
              ))}
            </ul>
          </div>
        </section>
